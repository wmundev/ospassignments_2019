...
void* alloc(size_t size)
{
        ...
        if (free_list.size == 0)
        {
                ...
                {
                        fprintf(stderr,
                                "Error: cannot add to the alloc list.\n");
                        return NULL;
                }
                /* return the memory allocated */
                return newchunk->memory;
        }
        /* if the free list is not empty, use the allocation approach to
         * allocate
         * the memory */
...
void dealloc(void* mem)
{
        ...
        if (!foundchunk)
        {
                /* if not found, there is an attempt to free memory we don't
                 * have */
                ...
                {
                        fprintf(stderr,
                                "Error: double free of memory detected.\n");
                }
#ifdef DEBUG
                /**
                 * if we are in debug mode, print the allocations and free list
                 *so we
...
void* worst_fit(size_t size)
{
        ...
        {
                /* when the largest chunk is not big enough or the list is
                 * empty,
                ...
                /* if we found a chunk but it's too small, readd it to the free
                 * list */
                if (foundchunk)
                {
                        if (!list_push_back(&free_list, foundchunk))
                        {
                                fprintf(
                                    stderr,
                                    "Error: could not readd to free list.\n");
                                abort();
                        }
                }
                ...
                {
                        fprintf(stderr,
                                "Error: could not add to the alloc list.\n");
                        abort();
                }
...
{
        /* we need to assign each pointer to the appropriate pointer type so we
         * can
        ...
        /* if we haven't set a best chunk yet, this is the best chunk we have
         * seend
         * so far */
        if (!bestchunk)
        {
        ...
        /* otherwise, if this chunk is smaller than the best so far but greater
         * than
