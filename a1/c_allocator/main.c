...
struct falsible_long strtonum(const char* input, const range* allowed_range)
{
        ...
        {
                fprintf(stderr, "Error: %s is not a number in the allowed "
                                "range of %ld-%ld.\n",
...
bool process_args(int argc, char* argv[], struct args_result* result)
{
        ...
        while (current_option = getopt(argc, argv, "s:a:"),
               current_option != EOF)
        {
...
int main(int argc, char* argv[])
{
        ...
        if (!populate_names(names))
        {
                if (errno == ENOENT)
                {
                        fprintf(stderr, "Error: the file \"first-names.txt\" "
                                        "is missing.\n");
                }
                else
                {
        ...
        for (iter_count = 0; iter_count < MAXITERS; ++iter_count)
        {
                ...
                printf("----------------\n");
                /* allocate memory for the int array and it elements and fill
                 * them */
                TIME_EVENT(intarray = (int**)alloc(sizeof(int*) * arraysize),
                           alloctime);
                for (count = 0; count < arraysize; ++count)
                {
                        TIME_EVENT(intarray[count] = (int*)alloc(sizeof(int)),
                                   alloctime);
                        *intarray[count] = rand();
                }
                /* allocate memory for the double array and its elements and
                 * fill them
                 */
                TIME_EVENT(dblarray =
                               (double**)alloc(sizeof(double*) * arraysize),
                           alloctime);
                for (count = 0; count < arraysize; ++count)
                {
                        TIME_EVENT(dblarray[count] =
                                       (double*)alloc(sizeof(double)),
                                   alloctime);
                ...
                for (count = 0; count < arraysize; ++count)
                {
                        ...
                        TIME_EVENT(namearray[count] =
                                       (char*)alloc(strlen(name) + 1),
                                   alloctime);
